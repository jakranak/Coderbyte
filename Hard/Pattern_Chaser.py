...
def PatternChaser(strParam):
  ...
  for c in strParam:
    if c in counts:
      ...
      counts[c] = 1
  # The longest possible pattern is the sum of all repeated characters by two. We start with this length string in our for loop below
  length = int(sum(doubles.values()) / 2)
  ...
  for l in range(length, 1, -1):
    ...
    for i in range(len(strParam) - l + 1):
      # If any substring repeats we return it. We also need to check that the two matching substrings don't overlap. So they must be seperated by at least l
